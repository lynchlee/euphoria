...
public class ReduceByKeyTranslator implements OperatorTranslator<ReduceByKey> {
  ...
  private static <IN, KEY, VALUE, OUT, W extends Window<W>> PCollection<Pair<KEY, OUT>> doTranslate(
    ...
    // ~ resolve types
    Preconditions.checkArgument(keyExtractor instanceof TypeAwareUnaryFunction,
        "Return type of key extractor must be specified");
    final TypeToken<KEY> keyType = ((TypeAwareUnaryFunction<IN, KEY>) keyExtractor)
        .getTypeHint()
        .getType();

    Preconditions.checkArgument(valueExtractor instanceof TypeAwareUnaryFunction,
        "Return type of value extractor must be specified");
    final TypeToken<VALUE> valueType = ((TypeAwareUnaryFunction<IN, VALUE>) valueExtractor)
        .getTypeHint()
        .getType();

    Preconditions.checkArgument(reducer instanceof TypeAwareReduceFunctor,
        "Return type of reducer must be specified");
    final TypeToken<OUT> outputType = ((TypeAwareReduceFunctor<VALUE, OUT>) reducer)
        .getTypeHint()
        .getType();

    // ~ get coders
    final CoderRegistry coderRegistry = context.getPipeline().getCoderRegistry();

    @SuppressWarnings("unchecked")
    final Coder<KEY> keyCoder = ExceptionUtils.unchecked(() -> coderRegistry
        .getCoder((TypeDescriptor<KEY>) TypeDescriptor.of(keyType.getType())));

    @SuppressWarnings("unchecked")
    final Coder<VALUE> valueCoder = ExceptionUtils.unchecked(() -> coderRegistry
        .getCoder((TypeDescriptor<VALUE>) TypeDescriptor.of(valueType.getType())));

    @SuppressWarnings("unchecked")
    final Coder<OUT> outputCoder = ExceptionUtils.unchecked(() -> coderRegistry
        .getCoder((TypeDescriptor<OUT>) TypeDescriptor.of(outputType.getType())));
